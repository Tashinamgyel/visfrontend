...
import { OwnershipTypes, Species, AnimalTypes, Breeds, Class, Medicines } from '@app/master-management/models/master';
...
@Component({
  selector: 'app-individual-sterialization-report',
  templateUrl: './individual-sterialization-report.component.html',
  styleUrls: ['./individual-sterialization-report.component.scss'],
})
export class IndividualSterializationReportComponent implements OnInit {
  sterializationReport: FormGroup;
  report: any;
  fromDate: string;
  toDate: string;
  ...
  initializeForm() {
    this.sterializationReport = this.fb.group({
  ...
  getReport() {
    ...
    Object.assign(reportRequest, this.sterializationReport.value);
    this.visMasterService.getIndivSterializationReport(reportRequest).subscribe((res) => {
      this.report = res;
