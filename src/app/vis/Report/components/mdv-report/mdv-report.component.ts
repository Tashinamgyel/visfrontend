...
@Component({
  selector: 'app-mdv-report',
  templateUrl: './mdv-report.component.html',
  styleUrls: ['./mdv-report.component.scss'],
})
export class MdvReportComponent implements OnInit {
  mdvReport: FormGroup;
  report: any;
  fromDate: string;
  toDate: string;
  ...
  initializeForm() {
    this.mdvReport = this.fb.group({
      ...
      catchMethodId: new FormControl('All'),
    });
  }
  ...
  getReport() {
    ...
    Object.assign(reportRequest, this.mdvReport.value);
    this.visMasterService.getMdvReport(reportRequest).subscribe((res) => {
      this.report = res;
